			...
			<div class="container-fluid">
				...
				<div class="row">
					...
					<div class="col-md-8 col-xs-12">
						<div class="white-box">
							<form class="form-horizontal form-material" action="<c:url value="/roles" />" method="post">
								...
								<c:if test="${messages != null}">
									<c:choose>
										<c:when test="${fn:startsWith(messages, 'success')}">
											<div style="color:green">${fn:substringAfter(messages, ':')}</div>				
										</c:when>
										<c:otherwise>
											<div style="color:red">${fn:substringAfter(messages, ':')}</div>	
										</c:otherwise>
									</c:choose>
								</c:if>
							</form>
